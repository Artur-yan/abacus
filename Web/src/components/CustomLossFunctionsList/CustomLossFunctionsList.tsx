...
import { useCustomLossFunctionsList } from '../../api/REUses';
...
const s = require('./CustomLossFunctionsList.module.css');
const sd = require('../antdUseDark.module.css');

interface ICustomLossFunctionsListProps {}

const CustomLossFunctionsList = React.memo((props: PropsWithChildren<ICustomLossFunctionsListProps>) => {
  ...
  const calcLink = useCallback((row) => ['/' + PartsLink.custom_loss_function_one + '/' + '-' + '/' + encodeURIComponent(row?.name), 'isEdit=1'], []);
        ...
        render: (text, row) => {
          ...
          const menu = (
            <Menu onClick={onClickVoid} getPopupContainer={popupContainerForMenu}>
              {row.notebookId && (
                <Menu.Item key={'editNB'} onClick={onClickGoLink.bind(null, ['/' + PartsLink.fast_notebook + '/' + '-' + '/' + encodeURIComponent(row.notebookId)])}>
                  Edit Notebook
                </Menu.Item>
              )}
              {
                <Menu.Item key={'del'}>
                  <ModalConfirm
                    ...
                    title={`Do you want to delete this Custom Loss Function '${row.name}'?`}
  ...
  const onClickDelete = (name) => {
    REClient_.client_().deleteCustomLossFunction(name, (err, res) => {
      if (err || !res?.success) {
  ...
  let list = useCustomLossFunctionsList();

  const calcKey = useCallback((r1) => r1.name, []);

  const onClickCreate = (e) => {
    Location.push('/' + PartsLink.custom_loss_function_one, undefined, 'isAdd=1');
  };
      ...
      <div className={sd.titleTopHeaderAfter} style={{ height: topAfterHeaderHH }}>
        ...
        >
          <span>
            ...
            <HelpIcon id={`customLossFunc_list_title`} style={{ marginLeft: '4px' }} />
          </span>
          <span
            css={`
              flex: 1;
...
export default CustomLossFunctionsList;
