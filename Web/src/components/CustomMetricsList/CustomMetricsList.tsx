...
import { useCustomMetricsList } from '../../api/REUses';
...
interface ICustomMetricsListProps {}

const CustomMetricsList = React.memo((props: PropsWithChildren<ICustomMetricsListProps>) => {
  ...
  const calcLink = useCallback((row) => ['/' + PartsLink.custom_metric_one + '/' + '-' + '/' + encodeURIComponent(row?.name), 'isEdit=1'], []);
        ...
        render: (text, row) => {
          ...
          const menu = (
            <Menu onClick={onClickVoid} getPopupContainer={popupContainerForMenu}>
              {row.notebookId && (
                <Menu.Item key={'editNB'} onClick={onClickGoLink.bind(null, ['/' + PartsLink.custom_metric_one + '/' + '-' + '/' + encodeURIComponent(row.name), 'notebookId=' + encodeURIComponent(row.notebookId)])}>
                  Edit Notebook
                </Menu.Item>
              )}
              {
                <Menu.Item key={'del'}>
                  <ModalConfirm
                    ...
                    title={`Do you want to delete this Custom Metric '${row.name}'?`}
  ...
  const onClickDelete = (name) => {
    REClient_.client_().deleteCustomMetric(name, (err, res) => {
      if (err || !res?.success) {
  ...
  let list = useCustomMetricsList();

  const calcKey = useCallback((r1) => r1.name, []);

  const onClickCreate = (e) => {
    Location.push('/' + PartsLink.custom_metric_one, undefined, 'isAdd=1');
  };
    ...
    <div className={sd.absolute + ' ' + sd.table} style={_.assign({ margin: '25px' }, { position: 'absolute', left: 0, top: 0, right: 0, bottom: 0 }) as CSSProperties}>
      <div className={sd.titleTopHeaderAfter} style={{ height: topAfterHeaderHH }}>
        ...
        >
          <span>
            Custom Metrics
            <HelpIcon id={`customMetric_list_title`} style={{ marginLeft: '4px' }} />
          </span>
          <span
            css={`
              flex: 1;
