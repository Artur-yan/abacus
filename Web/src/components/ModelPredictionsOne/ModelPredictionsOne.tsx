...
import ModelPredictionCompareWith, { ICompareFilePredOne } from '../ModelPredictionCompareWith/ModelPredictionCompareWith';
...
const s = require('./ModelPredictionsOne.module.css');
...
interface IModelPredictionsOneProps {
  paramsProp?: any;
  projects?: any;
  algorithms?: any;
  defDatasets?: any;
...
interface IModelPredictionsOneState {
  ...
  filesCompare?: ICompareFilePredOne[];
  dataChart?: any;
...
class ModelPredictionsOne extends React.PureComponent<IModelPredictionsOneProps & IModelPropsCommon, IModelPredictionsOneState> {
  private unDark: any;
  private chartdiv: HTMLDivElement;
  private explanationChartDiv: HTMLDivElement;
  private prepareChartTimer: any;
  private alreadyPreparedChart: boolean;
  private chart: any;
  private explanationChart: any;
  ...
  filesCompareLast: any;
  filesCompareCacheDataProcessed: any;
  filesCompareCacheDataProcessedError: any;
  dataFieldX: any;
  lastDataChart: any;
  ...
  constructor(props) {
    ...
    this.state = {
      sessionId: uuid.v1(),
      firstTime: true,
      predLenSel: Utils.tryParseInt(calcParam('predLenSel')) ?? undefined,
  ...
  componentDidUpdate(prevProps: Readonly<IModelPredictionsOneProps & IModelPropsCommon>, prevState: Readonly<IModelPredictionsOneState>, snapshot?: any): void {
    this.doMem();
  }
  ...
  refreshUrlWithParams = () => {
    ...
    if (projectId) {
      ...
      let params: any = {
        ...
        predLenSel: this.state.predLenSel,
  ...
  recreateChart = (useData?: any, forceUseData = false, fieldListEnabled = null, testFoldStartTs = null, filesCompare = null, selectedFieldValueId = null) => {
    if (!this.isM) {
      return;
    }

    if (this.filesCompareLast != filesCompare) {
      this.filesCompareLast = filesCompare;
      forceUseData = true;
    ...
    if (this.lastChartData == null) {
      ...
      if (this.explanationChart != null) {
        // @ts-ignore
        this.explanationChart.dispose?.();
        this.explanationChart = null;
      }
      $(this.explanationChartDiv).empty();
    }
    ...
    this.setState(
      ...
      () => {
        this.prepareChart(useData, undefined, usedCacheData, fieldListEnabled, testFoldStartTs, filesCompare, selectedFieldValueId);
      },
    );
  };
  ...
  prepareChart = (useData: any, max = 50, usedCacheData = false, fieldListEnabled = null, testFoldStartTs = null, filesCompare = null, selectedFieldValueId = null) => {
    if (!this.isM) {
      return;
    }
    ...
    if (!this.chartdiv) {
      ...
      if (max > 0) {
        this.prepareChartTimer = setTimeout(() => {
          this.prepareChart(useData, max - 1, usedCacheData, fieldListEnabled, testFoldStartTs, filesCompare, selectedFieldValueId);
        }, 40);
      }
      return;
    }
    ...
    const filePrefix = 'filesiuahiduhaisudhuiashdiuahsudh';
    ...
    Promise.all([
      ...
      .then((modules) => {
        ...
        let explanationChart = this.explanationChart;
        if (chart == null || willUseNewData) {
          ...
          explanationChart = am4core.create(this.explanationChartDiv, am4charts.XYChart);
          this.chart = chart;
          this.explanationChart = explanationChart;
          lastChartAxis = {};
        }
        $this.lastChartAxis = lastChartAxis;
        chart.paddingRight = 20;
        chart.paddingTop = 24;
        explanationChart.paddingRight = 20;
        explanationChart.paddingTop = 24;

        let lenPredShowCircle = !!useData.lenPredShowCircle;
        let dataChart = useData.data;

        if (!useData.dataAlreadyConverted && dataChart) {
          const testWindowCallResult = this.state.testWindowCallResult;
          if (dataChart && testWindowCallResult != null && _.isArray(testWindowCallResult) && testWindowCallResult.length > 0) {
            dataChart = [...dataChart];
            const c1 = useData.fieldX;
            testWindowCallResult.some((d1) => {
              let dt1 = d1[c1];
              let ind1 = dataChart.findIndex((v1) => v1[c1] === dt1);
              if (ind1 === -1) {
                let d2 = {};
                ...
                kk.some((k1) => {
                  ...
                  d2[(k1?.toLowerCase()?.[0] === 'p' ? '__' : '') + k1] = d1[k1];
                });
                dataChart.push(d2);
              } else {
                const kk = Object.keys(d1);
                kk.some((k1) => {
                  ...
                  dataChart[ind1][(k1?.toLowerCase()?.[0] === 'p' ? '__' : '') + k1] = d1[k1];
                });
              }
            });
          }
          ...
          const useOtherP = !this.isPredEndAtEnd(dataChart);

          if (testFoldStartTs != null && useData.fieldX && useData.field1 && !useOtherP) {
        ...
        //
        // const useOtherP = !this.isPredEndAtEnd(dataChart);
        if (!useData.dataAlreadyConverted) {
          let d1ind = _.findIndex(dataChart ?? [], (d1: any) => d1.__p50 != null || d1.p50 != null);
          if (d1ind > -1) {
            while (d1ind >= 0 && (dataChart?.[d1ind]?.__p50 != null || dataChart?.[d1ind]?.p50 != null)) {
              d1ind--;
            }

            if (dataChart[d1ind]) {
              dataChart = [...dataChart];
              dataChart?.some((d1, d1ind) => {
                if (d1 && d1._field1Removed != null) {
                  d1[useData.field1] = d1._field1Removed;
                }
                if (d1 && d1.__field1Removed != null) {
                  d1['_' + useData.field1] = d1.__field1Removed;
                }

                if (d1[useData.fieldX + '_bulletOpacity'] === 1) {
                  d1[useData.fieldX + '_bulletOpacity'] = 0;
                  delete d1[useData.fieldX + '_bulletOpacityLast'];

                  d1.dummyData = d1.dummyData ?? {};
                  d1.dummyData.bulletOpacity = 0;

                  // d1.p50 = d1.p50old;
                  // d1.__p50 = d1.__p50old;
                }
                delete d1.actualTrainIntDash3;
              });

              //
              dataChart[d1ind][useData.fieldX + '_bulletOpacity'] = 1;
              dataChart[d1ind][useData.fieldX + '_bulletOpacityLast'] = 1;

              if (this.isPredEndAtEnd(dataChart)) {
                if (dataChart[d1ind] != null && dataChart[d1ind][useData.field1] != null) {
                  dataChart[d1ind]._field1Removed = dataChart[d1ind][useData.field1];
                  dataChart[d1ind].__field1Removed = dataChart[d1ind]['_' + useData.field1];
                  delete dataChart[d1ind][useData.field1];
                  delete dataChart[d1ind]['_' + useData.field1];
                }
                if (dataChart[d1ind + 1] != null && dataChart[d1ind + 1][useData.field1] != null) {
                  dataChart[d1ind + 1]._actualRemoved = dataChart[d1ind + 1].actualTrainInt;
                  delete dataChart[d1ind + 1].actualTrainInt;
                }
              }

              dataChart[d1ind].dummyData = dataChart[d1ind].dummyData || {};
              dataChart[d1ind].dummyData.bulletOpacity = 1;

              // dataChart[d1ind].p50old = dataChart[d1ind].p50;
              // dataChart[d1ind].__p50old = dataChart[d1ind].__p50;

              // dataChart[d1ind][!useOtherP ? '__p50' : 'p50'] = dataChart[d1ind].actualTrainInt ?? dataChart[d1ind].actual;

              dataChart[d1ind].actualTrainIntDash3 = dataChart[d1ind].actualTrainInt ?? dataChart[d1ind].actual ?? dataChart[d1ind]['_' + useData.field1];
              if (dataChart[d1ind + 1] != null) {
                dataChart[d1ind + 1].actualTrainIntDash3 = dataChart[d1ind + 1].__p50 ?? dataChart[d1ind + 1].p50;
                // dataChart[d1ind + 1][!useOtherP ? '__p50' : 'p50'];
              }
            }
          }
        }
        ...
        let actualTrainIntDashUsed = false;
        let usedNewData = false;
        if (willUseNewData) {
          usedNewData = true;

          let dataList = dataChart;

          let ind = dataList.length - 1;
          let doSetActualMore = false,
            firstWithActualInd = null;
          while (ind > 0) {
            let d1 = dataList[ind];
            ...
            const kk = Object.keys(d1);
            kk.some((k1) => {
              if (_.startsWith(k1, 'p')) {
                anyP = true;
                return true;
              }
            });

            if (d1?._actual != null && firstWithActualInd == null) {
              firstWithActualInd = ind + 1;
            }

            if (!anyP) {
              doSetActualMore = true;
              ind++;
              break;
            }
          ...
          if (firstWithActualInd != null) {
            ind = firstWithActualInd;
            if (dataList[ind - 1] != null && dataList[ind] != null) {
              if (dataList[ind]._actual == null && dataList[ind - 1].p50 == null) {
                dataList[ind - 1].actualTrainIntDash = dataList[ind - 1]._actual;
                dataList[ind].actualTrainIntDash = dataList[ind].p50 ?? dataList[ind - 1]._actual;
                actualTrainIntDashUsed = true;
              }
            }
          }

          this.lastChartData = dataList;
        } else {
          let isAfterMaxPred = false;

          this.lastChartData = this.lastChartData || [];
          let dataList = [...this.lastChartData];
          ...
          if (!isAfterMaxPred) {
            ...
            dataChart?.some((d1) => {
              if (!Object.keys(d1).some((k1) => k1.length > 0 && k1[0]?.toLowerCase() === 'p')) {
                return false;
              }

              const dt1 = d1[useData.fieldX + '_lastInt'];
              const p = dataList.findIndex((d2) => d2[useData.fieldX + '_lastInt'] === dt1);
              if (p > -1) {
                dataList[p] = _.assign({}, d1, dataList[p]);

                if (p < ind) {
                  doSetActualMore = false;
                }
              }
            });

            let d1ind = _.findIndex(dataList ?? [], (d1: any) => d1.__p50 != null || d1.p50 != null);
            if (d1ind > -1) {
              while (d1ind >= 0 && (dataList?.[d1ind]?.__p50 != null || dataList?.[d1ind]?.p50 != null)) {
                d1ind--;
              }

              if (dataList[d1ind] != null) {
                //
                dataList[d1ind][useData.fieldX + '_bulletOpacity'] = 1;

                if (this.isPredEndAtEnd(dataList)) {
                  delete dataList[d1ind][useData.field1];
                  delete dataList[d1ind]['_' + useData.field1];
                  if (dataList[d1ind + 1] != null) {
                    dataChart[d1ind + 1]._actualRemoved = dataChart[d1ind + 1].actualTrainInt;
                    delete dataList[d1ind + 1].actualTrainInt;
                  }
                }

                dataList[d1ind].dummyData = dataList[d1ind].dummyData || {};
                dataList[d1ind].dummyData.bulletOpacity = 1;

                dataList[d1ind].actualTrainIntDash3 = dataList[d1ind]._actual ?? dataList[d1ind].actualTrainInt ?? dataList[d1ind].actual;
                if (dataList[d1ind + 1] != null) {
                  dataList[d1ind + 1].actualTrainIntDash3 = dataList[d1ind + 1].__p50 ?? dataList[d1ind + 1].p50;
                }
              }
            }
          }
        ...
        if (selectedFieldValueId) {
          ...
          const addOrSetValue = (f1id, itemId1, m1, m1unix, v1, cachedError1?) => {
            if (itemId1 === selectedFieldValueId) {
              const c1 = useData.fieldX;

              if (v1 != null && c1 != null && m1unix != null && m1unix !== 0) {
                let d1 = dictByDateNumber[m1unix];
                // dataChart.find(v1 => v1[c1 + '_lastInt'] === m1unix);
                if (!d1) {
                  d1 = {};
                  dataChart.push(d1);
                  anyAddedFiles = true;

                  d1[c1 + '_lastInt'] = m1unix;
                  d1[c1 + '_lastMoment'] = m1;
                  d1[c1] = m1.toDate();

                  dictByDateNumber[m1unix] = d1;
                }

                d1[filePrefix + f1id] = v1;

                //
                let actual2 = d1?.['__p50'] ?? d1?.['____p50'] ?? d1?.['p50'] ?? d1?.['_p50'];
                let pred2 = v1;
                let prePredAbacus2 = d1?.['_actualValue'] ?? d1?.['_actual'];

                if (actual2 != null && pred2 != null) {
                  cachedError1.sumActual += Math.abs(pred2);
                  cachedError1.sumDiff += Math.abs(pred2 - actual2);
                }
                if (actual2 != null && prePredAbacus2 != null && pred2 != null) {
                  cachedError1.sumActualAbacus += Math.abs(prePredAbacus2);
                  cachedError1.sumDiffAbacus += Math.abs(prePredAbacus2 - actual2);
                }
              }
            }
          };

          let anyAddedFiles = false;
          filesCompare?.some((f1: ICompareFilePredOne, f1ind) => {
            let cachedError1: { sumActualAbacus: number; sumDiffAbacus: number; sumActual: number; sumDiff: number; error: number; errorAbacus: number } = this.filesCompareCacheDataProcessedError?.[f1.id] ?? {
              sumActualAbacus: 0,
              sumActual: 0,
              sumDiffAbacus: 0,
              sumDiff: 0,
            ...
            const cachedData1 = this.filesCompareCacheDataProcessed?.[f1.id];
            if (cachedData1 != null) {
              cachedData1?.some((d1) => {
                const itemId1 = d1.itemId;
                const m1 = d1.m1;
                const m1unix = d1.m1unix;
                const v1 = d1.v1;

                addOrSetValue(f1.id, itemId1, m1, m1unix, v1, cachedError1);
              });

              return;
            }

            if (!Utils.isNullOrEmpty(f1.content)) {
              let content1 = f1.content;
              if (content1 != null) {
                if (content1 != null && _.isArray(content1) && content1.length > 0) {
                  let fileContentProcessed = [] as any[];
                  content1.some((s1) => {
                    let lineValues = s1;
                    if (lineValues.length >= 3) {
                      if (lineValues.length > 3) {
                        lineValues = lineValues.slice(0, 3);
                      }
                      if (lineValues.some((v1) => Utils.isNullOrEmpty(v1))) {
                        return;
                      }

                      //
                      let date1 = lineValues?.[1];
                      let date2 = Utils.tryParseInt(date1);
                      if (date2 != null) {
                        date1 = date2;
                      }
                      const v1 = Utils.tryParseFloat(lineValues?.[2]);

                      let m1;
                      if (_.isNumber(date1)) {
                        m1 = moment.unix(date1 / 1000);
                      } else {
                        if (_.isString(date1) && date1.indexOf('Z') === -1) {
                          m1 = moment.utc(date1);
                        } else {
                          m1 = moment(date1);
                        }
                      ...
                      const m1unix = m1 == null ? null : m1.unix() * 1000;

                      const itemId1 = lineValues?.[0];

                      if (Utils.isNullOrEmpty(itemId1) || m1unix == null || m1unix === 0 || v1 == null) {
                        return;
                      }
                  ...
                  if (fileContentProcessed != null && fileContentProcessed.length > 0) {
                    this.filesCompareCacheDataProcessed = this.filesCompareCacheDataProcessed ?? {};
                    this.filesCompareCacheDataProcessed[f1.id] = fileContentProcessed;
                  }
                  if (cachedError1 != null) {
                    this.filesCompareCacheDataProcessedError = this.filesCompareCacheDataProcessedError ?? {};
                    cachedError1.error = cachedError1.sumActual === 0 ? 0 : (cachedError1.sumDiff / cachedError1.sumActual) * 100;
                    cachedError1.errorAbacus = cachedError1.sumActualAbacus === 0 ? 0 : (cachedError1.sumDiffAbacus / cachedError1.sumActualAbacus) * 100;
                    this.filesCompareCacheDataProcessedError[f1.id] = cachedError1;
                  }
                }
              }
            }
          ...
          if (anyAddedFiles) {
            dataChart = dataChart?.sort((a, b) => {
              const av = a[useData.fieldX + '_lastInt'];
              const bv = b[useData.fieldX + '_lastInt'];
        ...
        if (dataChart) {
          ...
          dataChart.some((d1) => {
            ...
            d1['_actualValue'] = d1?._actualRemoved ?? d1?.actualTrainInt ?? (useData.field1 == null ? null : d1?.[useData.field1]) ?? d1?._actual;
        ...
        if (isNewChart) {
          // if(dateAxis?.renderer?.labels?.template!=null) {
          //   dateAxis.renderer.labels.template.location = -1.001;
          // }
        ...
        if (isNewChart) {
          onEndChangeAxisDate = (ev) => {
            ...
            let predictionStartT = minZoomed == null ? null : moment(minZoomed).tz(this.state.useTimezoneForAPI || 'UTC', false);
            let predictionEndT = maxZoomed == null ? null : moment(maxZoomed).tz(this.state.useTimezoneForAPI || 'UTC', false);
            if (this.isPredEndAtEnd(dataChart, predictionEndT) && this.state.predLenSel != null) {
              this.setState(
                {
                  predictionEnd: null,
                },
              ...
              if ((this.state.predictionStart != null && predictionStartT == null) || (this.state.predictionStart == null && predictionStartT != null) || !this.state.predictionStart.isSame(predictionStartT)) {
                if (!this.state.showTime && predictionStartT) {
                  predictionStartT = predictionStartT.startOf('day');
                }
                this.setState(
                  {
        ...
        if (filesCompare != null && filesCompare.length > 0 && selectedFieldValueId) {
          filesCompare.some((f1: ICompareFilePredOne, f1ind) => {
            fieldList1.push(filePrefix + f1.id);
          });
        }
        ...
        if (fieldList1 != null && _.isArray(fieldList1) && fieldList1.length > 0) {
          ...
          fieldList1.some((f1, f1ind) => {
            let isCompareFile = null,
              isFirstCompareFile = false;
            if (_.startsWith(f1, filePrefix)) {
              let st1 = f1.substring(filePrefix.length);
              if (st1 != null) {
                isCompareFile = filesCompare?.find((v1) => v1.id === st1);
                isFirstCompareFile = filesCompare?.[0] === isCompareFile;
              }
            }

            let isSeriesVisible = true;
            if (fieldListEnabled != null && isCompareFile == null) {
            ...
            if (isCompareFile != null) {
              isSeriesVisible = isCompareFile.isUse === true;
            }
            ...
            if (isNewChart) {
              if (useLegend) {
                ...
                } else if (isCompareFile) {
                  let errS1 = null;
                  let errFile1 = this.filesCompareCacheDataProcessedError?.[isCompareFile.id];
                  if (errFile1?.error != null) {
                    errS1 = errS1 ?? '';
                    errS1 += `\n[bold red line-height: 1.5]Error: ${Utils.decimals(errFile1?.error, 3)}%[/]`;
                  }
                  if (errFile1?.errorAbacus != null) {
                    errS1 = errS1 ?? '';
                    errS1 += `\n[bold red]Abacus Error: ${Utils.decimals(errFile1?.errorAbacus, 3)}%[/]`;
                  }
                  seriesN.name = 'File: ' + isCompareFile.filename + (errS1 ?? '');
                } else {
                  seriesN.name = f1;
                }
              }
              ...
              let c1: any = null;
              if (isCompareFile != null) {
                c1 = am4core.color(isFirstCompareFile ? compareFileColor : colorSet.next());
                seriesN.strokeWidth = 2;
                seriesN.strokeDasharray = '4,4';
              } else if (['p50', '__p50'].includes(f1?.toLowerCase())) {
            ...
            chart.events.on('ready' /*"datavalidated"*/, function () {
              ...
              fieldList1.some((f1, f1ind) => {
                ...
                if (isNewChart) {
                  ...
                  if (useLegend) {
                    if (isCompareFile) {
                      seriesN.name = 'File: ' + isCompareFile.filename;
                    } else {
                      seriesN.name = f1; //useData.seriesName2;
                    }
                  }
                  seriesN.dataFields.dateX = useData.fieldX;
                  seriesN.dataFields.valueY = (isCompareFile ? '' : '_') + f1;
        ...
        let explanationDataList = [];
        const explanationDataColumns = useData?.explanations ? Object.keys(useData?.explanations) : [];
        if (explanationDataColumns.length > 0) {
          explanationDataList = Object.entries(useData?.explanations?.[explanationDataColumns[0]]).map(([key, value]) => {
            const explanationData = {
              date: key,
              [explanationDataColumns[0]]: value,
            };

            for (let i = 1; i < explanationDataColumns.length; i++) {
              explanationData[explanationDataColumns[i]] = useData?.explanations?.[explanationDataColumns[i]][key];
            }
        ...
        let explanationDateAxis = explanationChart.xAxes.push(new am4charts.DateAxis());

        explanationDateAxis.baseInterval = {
          timeUnit: 'minute',
          count: 1,
        };
        explanationDateAxis.tooltipDateFormat = 'HH:mm, d MMMM';
        explanationDateAxis.dateFormatter.utc = true;

        explanationDateAxis.showOnInit = false;
        explanationDateAxis.keepSelection = true;

        let explanationValueAxis = explanationChart.yAxes.push(new am4charts.ValueAxis());

        explanationValueAxis.tooltip.disabled = true;
        explanationValueAxis.title.text = 'Feature Importance';

        explanationValueAxis.showOnInit = false;
        // explanationValueAxis.min = 0;
        explanationValueAxis.strictMinMax = true;

        explanationChart.cursor = new am4charts.XYCursor();
        explanationChart.cursor.lineY.opacity = 0;

        explanationChart.scrollbarX = new am4charts.XYChartScrollbar();

        explanationDataColumns.forEach((column, index) => {
          if (column === 'date') {
            return;
          }

          let series = explanationChart.series.push(new am4charts.LineSeries());
          ...
          const colorIndex = index > chatColorSet.length - 1 ? index % chatColorSet.length : index;
          const c1 = am4core.color(chatColorSet[colorIndex]);
          series.stroke = c1;
          series.fill = c1;
          series.strokeWidth = 2;

          // @ts-ignore
          explanationChart.scrollbarX.series.push(series);
        });

        explanationChart.legend = new am4charts.Legend();

        explanationChart.data = explanationDataList;

        explanationChart.events.on('ready', function () {
          // @ts-ignore
          explanationChart.scrollbarX.scrollbarChart.plotContainer.filters.clear();
          // @ts-ignore
          explanationChart.scrollbarX.unselectedOverlay.fillOpacity = 0.7;
        });

        explanationChart.events.on('datavalidated', function () {
          const chartDataLen = explanationDataList?.length;
          if (chartDataLen === 0) {
            return;
          }
          let zoomStart = explanationDataList?.[0]?.['date'];
          let zoomEnd = explanationDataList?.[chartDataLen - 1]?.['date'];
          if (chartDataLen > 30) {
            zoomStart = explanationDataList?.[chartDataLen - 30]?.['date'];
          }

          explanationDateAxis.zoomToDates(zoomStart, zoomEnd, false, true);
        });
      })
      .catch((e) => {
  ...
  onKeyPressId2 = (e) => {
    if (e.key?.toLowerCase() === 'enter') {
      this.onClickSelectedFieldValueId2(e);
    }
  };
  ...
  memGetChartData = memoizeOne((projectId, algoSelectValue, fieldValueSelectValue, rangeDatesSelectValue, sessionId, fieldListEnabled, testFoldStartTs, filesCompare, selectedFieldValueId, testWindowCall = false, predLenSel = null) => {
    if (Utils.isNullOrEmpty(algoSelectValue) || Utils.isNullOrEmpty(fieldValueSelectValue) || Utils.isNullOrEmpty(projectId)) {
      return;
    }

    if (predLenSel == null) {
      predLenSel = undefined;
    }
    ...
    const keyCache = algoSelectValue + ' ' + fieldValueSelectValue + ' ' + rangeDatesSelectValue + ' predLen:' + (predLenSel ?? '-');
    const inCache = testWindowCall && this.cacheChartData[keyCache];
    if (inCache && inCache.error != null) {
      this.setState({
        errorMsgPrediction: inCache.error || Constants.errorDefault,
      });
      this.recreateChart(null, true, undefined, undefined, undefined, undefined);
      return inCache;
    } else if (inCache && inCache.data != null && inCache.error == null && inCache.filesCompare === filesCompare) {
      if (!inCache.isRefreshing) {
        setTimeout(() => {
          ...
          this.recreateChart(inCache.data, true, fieldListEnabled, testFoldStartTs, filesCompare, selectedFieldValueId);
        }, 0);
      }
      return inCache;
    } else {
      ...
      if (predLenSel != null && _.isNumber(predLenSel)) {
        dataParams = dataParams ?? {};
        dataParams.numPredictions = predLenSel;
      }

      REClient_.client_()._predictForUI(algoSelectValue, dataParams, { explainPredictions: true }, null, (err, res) => {
        ...
        if (err) {
          obj1.error = err || Constants.errorDefault;
          lastUseData = null;
        ...
        if (this.lastCallChartData === uuid1) {
          if (!err && res.result) {
            ...
            obj1.filesCompare = filesCompare;

            // if(!testWindowCall && this.isChartReady===true) {
            //   let resR = res.result;
            //   let dataR = [];
            //   resR?.data?.some(d1 => {
            //     let r1 = {};
            //     resR?.fieldList?.some(s1 => {
            //       if(d1[s1]!=null) {
            //         r1[s1] = d1[s1];
            //       }
            //     });
            //     resR?.extra_table_columns.some(s1 => {
            //       if(d1[s1]!=null) {
            //         r1[s1] = d1[s1];
            //       }
            //     });
            //
            //     if(!_.isEmpty(r1)) {
            //       r1[resR.fieldX] = d1[resR.fieldX];
            //       dataR.push(r1);
            //     }
            //   });
            //
            //   this.setState({
            //     testWindowCallResult: dataR,
            //     testWindowCall: true,
            //   });
            //   return;
            // }

            this.recreateChart(obj1.error != null ? null : obj1.data, obj1.error != null ? true : undefined, fieldListEnabled, testFoldStartTs, filesCompare, selectedFieldValueId);
          } else {
            this.recreateChart(null, true, undefined, testFoldStartTs, filesCompare, selectedFieldValueId);
          }
        }
      }); //end run_mdoel
  ...
  memTestDatas = memoizeOne((optionsTestDatas) => {
    if (
      (this.state.selectedFieldValueId2 || this.state.selectedFieldValueId) == null ||
      (optionsTestDatas && optionsTestDatas.length > 0 && optionsTestDatas.find((o1) => o1.value === (this.state.selectedFieldValueId2 || this.state.selectedFieldValueId)) == null)
    ) {
      if (optionsTestDatas && optionsTestDatas.length > 0 && Utils.isNullOrEmpty(this.state.selectedFieldValueId2)) {
        setTimeout(() => {
          if (!this.isM) {
            return;
          }
  ...
  onFileDeleteCompare = (id) => {
    if (this.filesCompareCacheDataProcessed?.[id]) {
      delete this.filesCompareCacheDataProcessed[id];
    }
  };

  onFilesChangesCompare = (files: ICompareFilePredOne[]) => {
    this.setState({
      predictionEnd: null,
      filesCompare: files == null ? undefined : files,
    });
  };
  ...
  memPredLenOptions = memoizeOne((predLenSel) => {
  ...
  onChangePredLen = (option1) => {
    this.setState(
      {
        predLenSel: option1?.value ?? undefined,
      },
      () => {
        this.refreshUrlWithParams();
      },
  ...
  render() {
    ...
    let rangeDateByTestDataId: any = (optionsTestDatasRes ? optionsTestDatasRes.rangeDateByTestDataId : null) || {};
    let metricName = optionsTestDatasRes?.metricName;
    ...
    let testDatasSelectValueValue = this.state.selectedFieldValueId2 || this.state.selectedFieldValueId;
    if (testDatasSelectValueValue === '') {
      testDatasSelectValueValue = null;
    }
    ...
    let optionsRangesDatesRes = this.memDatasInfoProcessAndOptionsRanges(this.state.selectedFieldValueId2 || this.state.selectedFieldValueId, rangeDateByTestDataId);
    ...
    let didSetDates = this.memCustomDates(useCustomDates, algoSelectValue && algoSelectValue.value, testDatasSelectValueValue);
    ...
    if (this.state.useTimezoneForAPI == null) {
      ...
      this.memGetChartData(
        ...
        testDatasSelectValueValue,
        rangeDates,
        this.state.sessionId,
        fieldListEnabled1,
        useCustomDates?.testFoldStartTs,
        this.state.filesCompare,
        this.state.selectedFieldValueId2 || this.state.selectedFieldValueId,
        this.state.testWindowCall,
        this.state.predLenSel,
      );
    }
    ...
    const dataChartWindow = this.memDataChartWindow(this.state.dataChart, this.state.zoomWinMin, this.state.zoomWinMax);
    ...
    let optionsPredLen = this.memPredLenOptions(this.state.predLenSel);
    const deploymentDetails = this.memDeployOne(false)(this.props.deployments, projectId, optionsDeploysSel?.value);

    let modelTrainingType = deploymentDetails?.modelDeploymentConfig?.modelTrainingType;
                ...
                {isRefreshing !== true && foundProject1 && (
                  <div style={{ minHeight: '600px', padding: '25px 30px', marginBottom: '20px' }} className={sd.grayPanel}>
                    <div style={{ position: 'relative', textAlign: 'center', paddingTop: '10px', marginBottom: '10px' }}>
                      <div>
                        {deploymentDetails ? (
                          <AlgoNameAndTrainedModelTypeWrapper>
                            {deploymentDetails.algoName ? (
                              <span>
                                Algorithm: <AlgoNameContent>{deploymentDetails?.algoName}</AlgoNameContent>
                              </span>
                            ) : null}
                            {Boolean(modelTrainingType) === true ? (
                              <span>
                                Trained Model Type: <TrainModelTypeContent>{modelTrainingType}</TrainModelTypeContent>
                              </span>
                            ) : null}
                          </AlgoNameAndTrainedModelTypeWrapper>
                        ) : null}
                        ...
                        )}
                        {
                          <span style={{ marginRight: '50px' }}>
                            ...
                            <span style={{ display: 'inline-block', width: '120px' }}>
                              <SelectExt options={optionsPredLen} value={optionsPredLen?.find((o1) => o1.value === this.state.predLenSel)} onChange={this.onChangePredLen} />
                            </span>
                          </span>
                        }
                      </div>
                      ...
                      >
                        ...
                        <span
                          ...
                          style={{ marginRight: '5px' }}
                        >
                          {(optionsTestDatasRes?.testIdName || (foundProject1 ? foundProject1.name : null) || 'Item ID') + ' (' + metricName + '):'}
                        </span>
                        ...
                        <span style={{ width: '300px', display: 'inline-block' }}>
                          <SelectExt key={'s' + testDatasSelectValue?.value} value={testDatasSelectValue} options={optionsTestDatas} onChange={this.onChangeSelectFieldValue} isSearchable={true} menuPortalTarget={menuPortalTarget} />
                        </span>
                        <span
                          css={`
                            margin-left: 5px;
                        ...
                        <span style={{ width: '200px', display: 'inline-block', marginLeft: '10px' }}>
                          <Input onKeyPress={this.onKeyPressId2} placeholder={'Enter an ITEM_ID'} style={{ height: '35px', borderRadius: 0 }} value={this.state.selectedFieldValueId2Bounce} onChange={this.onChangeSelectedFieldValueId2} />
                        </span>
                        <Button className={sd.detailbuttonblueBorder} style={{ height: '35px', marginLeft: '5px' }} ghost onClick={this.onClickSelectedFieldValueId2}>
                          Go
                        </Button>

                        {
                          <span
                            ...
                            `}
                          >
                            <ModelPredictionCompareWith deploymentId={this.props.selectedAlgoId} onFilesChanges={this.onFilesChangesCompare} onFileDelete={this.onFileDeleteCompare} />
                          </span>
                        }
                    ...
                    <div style={{ textAlign: 'center', borderTop: '1px solid ' + Utils.colorA(0.2), paddingTop: '10px', marginBottom: 0, position: 'relative' }}>
                      ...
                      <div>
                        <RefreshAndProgress isRelative errorMsg={this.state.errorMsgPrediction} isRefreshing={isRefreshingChart}>
                          ...
                          ></div>
                          {lastUseData?.explanations && (
                            <div css={'text-align: left;'}>
                              <div css={'margin-left: 50px; font-size: 16px;'}>Explanations</div>
                              <div
                                ref={(r1) => {
                                  this.explanationChartDiv = r1;
                                }}
                                style={{ height: /*Math.max(440, height-topHH)*/ chartHH + 180 + 'px' }}
                              ></div>
                            </div>
                          )}
...
const AlgoNameAndTrainedModelTypeWrapper = styled.div`
  font-family: Roboto;
  font-size: 12px;
  font-weight: bold;
...
const AlgoNameContent = styled.span`
  font-size: 11px;
  font-weight: bold;
  color: #ffffff;
  text-transform: none;
...
const TrainModelTypeContent = styled(AlgoNameContent)``;
