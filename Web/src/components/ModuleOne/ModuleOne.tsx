...
import { useModuleOne, useNotebook } from '../../api/REUses';
...
const s = require('./ModuleOne.module.css');
const sd = require('../antdUseDark.module.css');

interface IPythonFunctionOneProps {}

const ModuleOne = React.memo((props: PropsWithChildren<IPythonFunctionOneProps>) => {
  ...
  const moduleOne = useModuleOne(name);
  const alreadyInitEdit = useRef(false);
  const editNotebookId = moduleOne?.notebookId;
  ...
  useEffect(() => {
    ...
    if (isEdit && moduleOne != null) {
      alreadyInitEdit.current = true;

      let vv: any = {
        name: moduleOne?.name,
        problemType: moduleOne?.problemType,
        sourceCode: moduleOne?.codeSource?.sourceCode,
      };

      form?.setFieldsValue(vv);
    }
  }, [isEdit, moduleOne, form]);
  ...
  const handleSubmit = (values) => {
    if (!isEdit) {
      REClient_.client_().createModule(values.name, (err, res) => {
        if (err || !res?.success) {
          REActions.addNotificationError(err || Constants.errorDefault);
        } else {
          StoreActions.listModules_();
          Location.push('/' + PartsLink.module_one + '/' + '-' + '/' + encodeURIComponent(values.name), undefined, 'notebookId=' + encodeURIComponent(res?.result?.notebookId));
        }
      });
    } else {
      Location.push('/' + PartsLink.modules_list + '/' + '-');
    }
  };
      ...
      <div style={{ margin: '30px auto', maxWidth: '1000px', color: Utils.colorA(1) }}>
        {
          <div>
            <div className={sd.titleTopHeaderAfter} style={{ height: topAfterHeaderHH }}>
              <span>
                {'Module'}
                <HelpIcon id={'module_edit_one'} style={{ marginLeft: '4px' }} />
              </span>
            </div>
          </div>
        }
              ...
              <FormExt layout={'vertical'} form={form} onChange={onFormChange} onFinish={handleSubmit} initialValues={{}}>
                {
                  <Form.Item
                    normalize={(value, prevValue, allValues) => {
                      return value?.toLowerCase()?.replace(/[^0-9A-Za-z_]/gi, '');
                    }}
                    name={'name'}
                    ...
                    label={
                      <span style={{ color: Utils.isDark() ? 'white' : 'black' }}>
                        Name:
                        <HelpIcon id={'module_name'} style={{ marginLeft: '4px' }} />
                      </span>
                    }
                  >
                    <Input
                ...
                {isEdit && (
                  ...
                  >
                    <Form.Item
                      ...
                      label={
                        <span style={{ color: Utils.isDark() ? 'white' : 'black' }}>
                          {'Source Code'}:<HelpIcon id={'module_code'} style={{ marginLeft: '4px' }} />
                        </span>
                      }
                    >
                      <EditorElemForFeatureGroup readonly allowResizeHeight={'hh_edit_code_module'} height={400 - 30 - 90} lang={'python'} />
                    </Form.Item>
                  </div>
                )}
                ...
                {isEdit && editNotebookId != null && (
                  <Form.Item
                    label={
                      <span style={{ color: Utils.isDark() ? 'white' : 'black' }}>
                        Notebook:
                        <HelpIcon id={'module_notebook'} style={{ marginLeft: '4px' }} />
                      </span>
                    }
                  >
                    <Link to={['/' + PartsLink.module_one + '/-/' + encodeURIComponent(name), 'fromEdit=1&notebookId=' + encodeURIComponent(editNotebookId)]} showAsLinkBlue usePointer>
                      <span>
