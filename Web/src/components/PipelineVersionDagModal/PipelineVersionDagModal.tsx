...
const styles = require('./PipelineVersionDagModal.module.css');
...
const NodeTemplate = React.memo(({ data, selectedShapeKeys }: PropsWithChildren<NodeTemplateProps>) => {
  const { stepName, stepId, functionInputMappings, stepOutputMappings, status, error } = data?.dataItem || {};
  ...
  const nodeStyles = {
    ...
    borderLeft: `4px solid ${isNodeSelected ? NodeTypeColors.SELECTED_NODE : NodeTypeColors.DEFAULT}`,
  };

  if (status == 'FAILED') {
    nodeStyles.borderLeft = `8px solid ${NodeTypeColors.FAILED}`;
  } else if (status == 'COMPLETE') {
    nodeStyles.borderLeft = `8px solid ${NodeTypeColors.COMPLETE}`;
  }

  const title = (
    <div className={styles.tooltipTitle}>
      <span className={styles.fgIdTxt}>{`Step: ${stepName}`}</span>
      <span style={{ color: NodeTypeColors[status] || '#ffffff' }}>{`${status}`}</span>
      {error != null ? (
        <>
          <div>
            <span className={styles.paramsTitle}>{`Error:`}</span>
          </div>
          <div>
            <span className={styles.dateCreatedTxt}>{`${error}`}</span>
          </div>
        </>
      ) : null}
...
interface PipelineVersionDagProps {
  nodes: any[];
  connections: any[];
  pipelineName: string;
}

const PipelineVersionDag = React.memo(({ nodes, connections, pipelineName }: PropsWithChildren<PipelineVersionDagProps>) => {
...
interface PipelineVersionDagModalProps {
  pipelineVersion: string;
  modalName?: string;
}

const PipelineVersionDagModal = React.memo(({ pipelineVersion, modalName }: PipelineVersionDagModalProps) => {
  const [pipeline, setPipelineVersion] = React.useState(null as any);
  ...
  const onShow = async () => {
    ...
    try {
      const response = await REClient_.promises_().describePipelineVersion(pipelineVersion);
      if (response?.error || !response?.success) throw new Error(response?.error);
      setPipelineVersion(response?.result);
    } catch (error) {
  ...
  React.useEffect(() => {
    ...
    const newNodes =
      pipeline?.stepVersions?.map?.((step) => ({
        stepName: step?.stepName,
        stepId: step?.pipelineStepId,
        functionInputMappings: step?.functionVariableMappings,
        stepOutputMappings: step?.outputVariableMappings,
        id: step?.stepName,
        status: step?.status,
        error: step?.error,
      })) || [];
  ...
  const content = (
    <div style={{ height: Heights.diagramContainer }}>
      <PipelineVersionDag nodes={nodes} connections={connections} pipelineName={pipeline?.pipelineName} />
    </div>
  );
...
export default PipelineVersionDagModal;
