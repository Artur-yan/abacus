...
export default function CustomSelect(props: CustomSelectProps) {
  const { value, grid = true, optionsList, setValue, locale, className, humanizeLabels, disabled, readOnly, leadingZero, clockFormat, period, unit, ...otherProps } = props;
  const [open, setOpen] = useState(false);

  useEffect(() => {
    Array.from(document.getElementsByClassName('ant-select-selection-search-input')).forEach((element: Element) => {
  ...
  const renderTag = useCallback(
    (props) => {
      ...
      return <div>{testEveryValue[1] ? `${locale.everyText || DEFAULT_LOCALE_EN.everyText} ${testEveryValue[1]}` : cronValue}</div>;
    },
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  const doubleClick = useCallback(
    (newValueOption: number) => {
      if (newValueOption !== 0 && newValueOption !== 1) {
        ...
        const oldValueEqualNewValue = value && newValue && value.length === newValue.length && value.every((v: number, i: number) => v === newValue[i]);
  ...
  return (
    <Select
      ...
      disabled={disabled}
      dropdownAlign={
        (unit.type === 'minutes' || unit.type === 'hours') && period !== 'day' && period !== 'hour'
          ? {
