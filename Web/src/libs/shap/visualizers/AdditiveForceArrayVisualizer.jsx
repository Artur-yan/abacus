...
import { cloneDeep, debounce, each, filter, findKey, keys, map, max, min, range, rangeRight, sortBy, sum } from 'lodash';
...
class AdditiveForceArrayVisualizer extends React.Component {
  ...
  componentDidMount() {
    ...
    this.hoverxTitle = this.svg.append('text').attr('text-anchor', 'middle').attr('opacity', 0.6).attr('font-size', '12px');
    ...
    this.svg.on('click', () => Utils.error('This original index of the sample you clicked is ' + this.nearestExpIndex));
  ...
  mouseMoved() {
    ...
    if (this.props.explanations) {
      ...
      let negFeatures = [];
      lastPos = undefined;
      this.currNegOrderedFeatures?.some((i) => {
        let d = nearestExp.features[i];
  ...
  draw() {
    ...
    this.props.explanations?.some((e) => {
      Object.keys(e.features ?? {}).some((k) => {
    ...
    let options = ['sample order by similarity', 'sample order by output value', 'original sample ordering'].concat(this.singleValueFeatures.map((i) => this.props.featureNames[i]));
  ...
  internalDraw() {
    ...
    let isTimeScale = xsort === 'sample order by key' && this.props.ordering_keys_time_format != null;
    if (isTimeScale) {
    ...
    if (xsort === 'sample order by similarity') {
      ...
      if (typeof xvals[0] == 'string') {
        Utils.error('Ordering by category names is not yet supported.');
        return;
      }
      let xmin = min(xvals);
      let xmax = max(xvals);
    ...
    for (let ind = 0; ind < N; ++ind) {
      ...
      let pos = -totalNegEffects,
        i;
      this.currPosOrderedFeatures?.some((i) => {
    ...
    let featureLabels = [];
    ['pos', 'neg'].some((featType) => {
      this.currUsedFeatures?.some((ind) => {
        let starti = 0,
          endi = 0,
          boxWidth = 0,
