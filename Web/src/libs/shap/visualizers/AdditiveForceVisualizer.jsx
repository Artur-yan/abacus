...
import { debounce, each, filter, findIndex, map, sortBy, sum } from 'lodash';
...
class AdditiveForceVisualizer extends React.Component {
  ...
  componentDidMount() {
    ...
    this.colors.map((c, i) => {
      let grad = this.svg
        ...
        .attr('x2', '0%')
        .attr('y2', '100%');
      grad.append('stop').attr('offset', '0%').attr('stop-color', c).attr('stop-opacity', 0.6);
      ...
      let grad2 = this.svg
        ...
        .attr('x2', '0%')
        .attr('y2', '100%');
      grad2.append('stop').attr('offset', '0%').attr('stop-color', c).attr('stop-opacity', 0.5);
      grad2.append('stop').attr('offset', '100%').attr('stop-color', c).attr('stop-opacity', 0);
    });
  ...
  draw() {
    ...
    blocks
      ...
      .on('mouseover', (d) => {
        if (scale(Math.abs(d.effect)) < scale(totalEffect) / 50 || scale(Math.abs(d.effect)) < 10) {
          let x = scale(d.x) + scaleOffset;
          let w = scale(Math.abs(d.effect));
          this.hoverLabel
            .attr('opacity', 1)
    ...
    let filteredData = filter(data, (d) => {
      return scale(Math.abs(d.effect)) > scale(totalEffect) / 50 && scale(Math.abs(d.effect)) > 10;
    });
    ...
    labels.attr('x', (d) => scale(d.textx) + scaleOffset + (d.effect > 0 ? -d.textWidth / 2 : d.textWidth / 2)).attr('text-anchor', 'middle'); //d => d.effect > 0 ? 'end' : 'start');
    ...
    blockDividers
      ...
      .attr('stroke', (d, i) => {
        if (joinPointIndex === i + 1 || Math.abs(d.effect) < 1e-8) {
          ...
          return this.brighterColors[0];
        } else {
          return this.brighterColors[1];
        }
      });
